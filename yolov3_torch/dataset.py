...
class YOLODataset(Dataset):
    def __init__(
        ...
        def __getitem__(self, index: int):
            label_path = label_dir / self.annotations.iloc[index, 1]
            bboxes = np.roll(np.loadtxt(fname=label_path, delimiter=" ", ndmin=2), -1, axis=1).tolist() # (x, y, w, h, c)
            img_path = img_dir / self.annotations.iloc[index, 0]
            ...
            for box in bboxes:
                ...
                for i_anchor in anchors_rank:
                    i_scale = i_anchor // self.num_anchors_per_scale
